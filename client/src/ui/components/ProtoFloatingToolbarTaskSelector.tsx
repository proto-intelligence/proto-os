...
/*
 * Documentation:
 * Proto Floating Toolbar Task Selector — https://app.subframe.com/61fe868f53c4/library?component=Proto+Floating+Toolbar+Task+Selector_60edd94e-6f8a-4e47-91e5-a2eb95098633
 */
    ...
    >
      <div
        className={SubframeUtils.twClassNames(
          "group/3ae1d4dd flex h-8 w-full cursor-pointer items-center gap-1 rounded-md px-3 hover:bg-neutral-100 active:bg-neutral-50 data-[highlighted]:bg-brand-50",
          className
        )}
        ref={ref as any}
      >
        <ProtoFloatingToolbarTaskSelector.ItemText className="h-auto grow shrink-0 basis-0">
          {children || value}
        </ProtoFloatingToolbarTaskSelector.ItemText>
        <FeatherCheck className="hidden text-body font-body text-default-font group-hover/3ae1d4dd:hidden group-data-[state=checked]/3ae1d4dd:inline-flex group-data-[state=checked]/3ae1d4dd:text-brand-600" />
      </div>
    </SubframeCore.Select.Item>
  );
});
  ...
  return (
    <SubframeCore.Select.Trigger asChild={true} {...otherProps}>
      ...
      >
        ...
        <ProtoFloatingToolbarTaskSelector.TriggerValue
...
interface ProtoFloatingToolbarTaskSelectorRootProps
...
const ProtoFloatingToolbarTaskSelectorRoot = React.forwardRef<
  HTMLElement,
  ProtoFloatingToolbarTaskSelectorRootProps
>(function ProtoFloatingToolbarTaskSelectorRoot(
  {
    disabled = false,
    error = false,
    variant = "outline",
  ...
  return (
    ...
    >
      <div
        className={SubframeUtils.twClassNames(
          "group/60edd94e flex cursor-pointer flex-col items-start gap-1",
          className
        )}
        ref={ref as any}
        {...otherProps}
      >
        <div
          className={SubframeUtils.twClassNames(
            "flex h-8 w-full flex-none flex-col items-start rounded-md border border-solid border-neutral-border bg-default-background group-focus-within/60edd94e:border group-focus-within/60edd94e:border-solid group-focus-within/60edd94e:border-brand-primary",
            {
              "border border-solid border-neutral-100 bg-neutral-100 group-hover/60edd94e:border group-hover/60edd94e:border-solid group-hover/60edd94e:border-neutral-border group-hover/60edd94e:bg-neutral-100":
                variant === "filled",
...
export const ProtoFloatingToolbarTaskSelector = Object.assign(
  ProtoFloatingToolbarTaskSelectorRoot,
  {
    Item,
    TriggerValue,
    Content,
